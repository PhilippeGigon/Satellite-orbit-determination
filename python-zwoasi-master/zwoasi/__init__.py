...
class _ASI_SUPPORTED_MODE(c.Structure):
    ...
    def get_dict(self):
        ...
        base_dict['SupportedCameraMode'] = [
            int(x) for x in base_dict['SupportedCameraMode']]
        return base_dict
...
def init(library_file=None):
    ...
    zwolib.ASIGetCameraProperty.argtypes = [
        c.POINTER(_ASI_CAMERA_INFO), c.c_int]
    ...
    zwolib.ASISetROIFormat.argtypes = [
    ...
    zwolib.ASIGetDataAfterExp.argtypes = [
        c.c_int, c.POINTER(c.c_char), c.c_long]
    ...
    zwolib.ASIGetCameraSupportMode.argtypes = [
        c.c_int, c.POINTER(_ASI_SUPPORTED_MODE)]
